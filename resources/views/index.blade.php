    ...
    <div class="container py-3">
        ...
        <div>
            @if (count($user_names) > 0)
                ...
                <table style="width:100%;" class="table table-bordered ">
                    ...
                    @foreach ($user_names as $item)
                        <tr class="table-group-divider">
                            ...
                            <td>{{ $item->age }}</td>
                            <td>{{ $item->gender }}</td>
                            <td><a href="{{ route('edit', $item->id) }}" name="edit_in" id="edit_in" class="btn"
                                    style="background-color: #445D48; color: #D6CC99;">แก้ไข</a></td>
                            <td><a href="{{ route('delete', $item->id) }}" name="de_in" id="de_in" class="btn"
                                    style="background-color: #001524; color: #D6CC99;"
                ...
                {{ $user_names->links() }}
            @else
                <h2 style="color: #001524;">ไม่พบรายชื่อ</h2>
            @endif
        </div>
    </div>
</body>
